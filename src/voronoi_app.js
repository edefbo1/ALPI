...
class voronoi_app {
  constructor() {
    ...
    if (config === null) {
      ...
      if (config.version == this.version) {
        ...
        console.log(
          `version mismatch (localstorage = ${config.version} , loaded page = ${this.version}) reinitialising`
        );
        init_needed = true;
      }
    }
  ...
  compute_voronoi() {
    this.diagram.compute(this.seeds.get_seeds(), {
      xl: 0,
      xr: parseFloat(this.width),
      yt: 0,
      yb: parseFloat(this.height),
    });
    this.draw();
  }
  ...
  load_dropped_svg(reader) {
    ...
    reader.onloadend = function (e) {
      vor_context.shape.load_path(this.result, vor_context);
    };
  }
  ...
  load_dropped_seeds(reader) {
    ...
    reader.onloadend = function (e) {
      ...
      const is_loaded = vor_context.seeds.load(result, {
        context: vor_context,
      });
      if (!is_loaded) {
