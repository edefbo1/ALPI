...
function onWheel(e) {
  ...
  const main_element = e.target.classList.contains("main")
    ? e.target
  ...
  const min_width = Math.round(
    main_element.getAttribute("data-side-min") * scale
  );
...
class Grid {
  ...
  constructor(parent, grid_side, max_sides = 2) {
    ...
    this.main_div = scale_grid(parent, this.sheet, {
      grid_side: grid_side,
      max_sides: max_sides,
    });
