...
function menu_export(parent) {
  br(parent);
  let [ecol0, ecol1, ecol2, ecol3, ecol4] = bs.cols(parent, 5, [
    "col-3",
    "col-1",
    "col-2",
    "col-2",
...
function menu_shape_space_min(parent) {
  ...
  let space_label = html(
    parent,
    /*html*/ `<a style="margin:10px">Space between cells ${vor.cells_space}</a>`
  );
  ...
  let in_label = html(
    parent,
    /*html*/ `<a style="margin:10px">min cell edge ${vor.min_edge}</a>`
  );
...
function menu_generate_view(parent) {
  ...
  bs.dropdown(parent, "Select Map", list, list, (e) => {
    vor.update({
      map: e.target.getAttribute("data-label"),
      w: vor.width,
      h: vor.height,
    });
  });
...
function menu_svg_size(parent) {
  ...
  let in_width = bs.input_text(
    parent,
    "in_width",
    `${vor.max_width} max width`,
    "w-100"
  );
  ...
  let in_height = bs.input_text(
    parent,
    "in_height",
    `${vor.max_height} max height`,
    "w-100"
  );
  ...
  let label_cost = html(
    parent,
    /*html*/ `<a style="margin:5px">Map Cost Vs Dist ${scfg.map_vs_dist}</a>`
  );
